...
const generateNextEmployeeCode = async (employeeRepo) => {
...
const isEmployeeCodeConflict = (err) => {
    const dbError = err;
...
router.get("/code/:employeeCode", auth_1.authenticateToken, async (req, res) => {
    try {
        const employeeRepo = data_source_1.AppDataSource.getRepository(Employee_1.Employee);
...
router.put("/code/:employeeCode", auth_1.authenticateToken, auth_1.isAdmin, async (req, res) => {
    try {
        const employeeRepo = data_source_1.AppDataSource.getRepository(Employee_1.Employee);
        const employee = await employeeRepo.findOne({
            where: { employeeCode: req.params.employeeCode }
...
router.delete("/code/:employeeCode", auth_1.authenticateToken, auth_1.isAdmin, async (req, res) => {
    try {
        const employeeRepo = data_source_1.AppDataSource.getRepository(Employee_1.Employee);
        const employee = await employeeRepo.findOne({
            where: { employeeCode: req.params.employeeCode }
...
router.post("/", auth_1.authenticateToken, auth_1.isAdmin, async (req, res) => {
    try {
        const { firstName, lastName, email, phone, designation, department, dateOfJoining, pan, aadhaar, bankAccount, bankIFSC, 
        // Salary details
        basicPay, hra, da } = req.body;
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
            try {
                completeEmployee = await data_source_1.AppDataSource.transaction(async (manager) => {
                    const employeeRepo = manager.getRepository(Employee_1.Employee);
                    const salaryRepo = manager.getRepository(Salary_1.Salary);
